...
GroundContactEstimator::GroundContactEstimator(pcl::PointCloud<pcl::PointXYZ> cloud)
{
    ...
    world_kdtree.setInputCloud(world_pcl_ptr);
...
float vectorSimilarity(pcl::PointXYZ support_point_1, pcl::PointXYZ support_point_2,
    ...
    float angle = fmin(angle1, angle2); //[degree]
    ...
    float distance = fmin(distance1, distance2); //[m]
...
float GroundContactEstimator::flatGroundPositionRating()
{
    ...
    std::vector<float> pos_rating = computeForceAngleStabilityMetric(center_mass, projected_robot_hull);
    std::vector<float>::iterator it_min_pos_rating = std::min_element(pos_rating.begin(), pos_rating.end());
    return *it_min_pos_rating;
}
...
bool GroundContactEstimator::fillRobotPointcloud(const pcl::PointXYZ& p0, const pcl::PointXYZ& p1, const pcl::PointXYZ& p2, const pcl::PointXYZ& p3, unsigned int& highest_Point_idx){
    ...
    const pcl::PointXYZ center = pcl::PointXYZ(0.25*(p0.x+p1.x+p2.x+p3.x),0.25*(p0.y+p1.y+p2.y+p3.y),0.25*(p0.z+p1.z+p2.z+p3.z));
    const float radius = distanceXY(p0, center);

    pcl::PointCloud<pcl::PointXYZ>::Ptr local_cloud (new pcl::PointCloud<pcl::PointXYZ>);

    std::vector<int> pointIdxRadiusSearch;
    std::vector<float> pointRadiusSquaredDistance;
    world_kdtree.radiusSearch(center, radius, pointIdxRadiusSearch, pointRadiusSquaredDistance);
    for (size_t i = 0; i < pointIdxRadiusSearch.size (); ++i)
        local_cloud->push_back(world_pcl_ptr->points[pointIdxRadiusSearch[i]]);

    for (unsigned int i = 0; i < local_cloud->size(); i++)
    {
        const  pcl::PointXYZ& pp= local_cloud->at(i);
...
bool GroundContactEstimator::fillRobotPointcloud(const pcl::PointXYZ& p0, const pcl::PointXYZ& p1, const pcl::PointXYZ& p2, const pcl::PointXYZ& p3, float& max_height){
...
{    
    ...
    if(!fillRobotPointcloud(p0, p1, p2, p3, highest_Point_idx))
    {
        ROS_WARN("[ground contact estimator] No points for ground contact estimation available. Planning into empty space?");
        return false;
    }
    ...
    for (unsigned int i = 0; i < robot_pcl->size(); i++)
    {
        ...
        if(fabs(p.z-p_max_height.z)<0.03)
        {
    ...
    while (true){ // find SupportPoints ; runs multiple times if unstable axis occur
        ...
        pcl::PointXYZ tip_over_axis_direction = (crossProduct(pcl::PointXYZ(support_point_1.x-flat_robot_center.x,support_point_1.y-flat_robot_center.y,0),pcl::PointXYZ(0,0,1)));
        ...
        bool point_evaluated = findSupportPoint(tip_over_axis_point,
                                                tip_over_axis_direction,
                                                *robot_pcl,
                                                tip_over_direction,
                                                support_point_2);
        if (!point_evaluated){
            ROS_WARN("[ground contact estimator] no support_point found 2..., counter = %i ", counter);
        ...
        const pcl::PointXYZ tip_over_axis_direction_3= subtractPoints(support_point_2, support_point_1);
        pcl::PointXYZ tip_over_direction_3 ;
        if(ccw(support_point_1,support_point_2,flat_robot_center)>0)
            tip_over_direction_3 = crossProduct(tip_over_axis_direction_3, pcl::PointXYZ(0,0,-1));
        else
            tip_over_direction_3 = crossProduct(tip_over_axis_direction_3, pcl::PointXYZ(0,0,1));

        point_evaluated = findSupportPoint(tip_over_axis_point_3,
                                           tip_over_axis_direction_3,
                                           (*robot_pcl),
    ...
    if (tip_over){
        ...
        for (unsigned int i = 0; i < rating.size(); i++){
            if (check_each_axis || rating.at(i) < invalid_rating){ // instabil
                ...
                // wenn gekippt und es hier stabiler ist, Ã¼bernehme den schlechtesen Wert der gekippten Position an
...
void GroundContactEstimator::extractRobotPointcloud(const pcl::PointXYZ& flat_robot_center, const float orientation, float& max_height)
{
    // Points at the edges of the robot (flat ground)
    pcl::PointXYZ p0, p1, p2, p3; // Edge Points
    computeRobotCornerPoints(flat_robot_center, orientation, p0,p1,p2,p3);
    fillRobotPointcloud(p0, p1, p2, p3, max_height);

}

void GroundContactEstimator::computeSupportPoint1(const pcl::PointXYZ& flat_robot_center,float max_height,  pcl::PointXYZ& support_point_1)
{
    float min_dist=-1.0;
    int support_point_1_idx = 0;
    for (unsigned int i = 0; i < robot_pcl->size(); i++)
    {
        pcl::PointXYZI& p = robot_pcl->at(i);
        if(((p.z-max_height)<0.03)&&((p.z-max_height)>-0.03))
        {
            float dist = sqrt((p.x-flat_robot_center.x)*(p.x-flat_robot_center.x)+(p.y-flat_robot_center.y)*(p.y-flat_robot_center.y));
            if(min_dist<0.0 || dist<min_dist)
            {
    ...
    support_point_1 = pcl::PointXYZ(robot_pcl->at(support_point_1_idx).x,robot_pcl->at(support_point_1_idx).y,robot_pcl->at(support_point_1_idx).z);

}

bool GroundContactEstimator::computePositionRatingRefactoring(const pcl::PointXYZ& flat_robot_center,
                                                              const float orientation,
                                                              pcl::PointXYZ& robot_point_center,
                                                              pcl::PointXYZ& robot_point_0,
                                                              pcl::PointXYZ& robot_point_1,
                                                              ...
                                                              int viewport_1, int viewport_2, int viewport_3, int viewport_4, bool use_visualization)
{
    ...
    unstable_axis = 0;
    float max_height;
    extractRobotPointcloud(flat_robot_center, orientation, max_height);
    ...
    pcl::PointXYZ support_point_1;
    computeSupportPoint1(flat_robot_center, max_height, support_point_1);
    ...
    /*computeRobotPosition(support_point_1, support_point_2, support_point_3, p0, p1, p2, p3,
                         ...
                         normal,
                         robot_point_0, robot_point_1, robot_point_2, robot_point_3,
                         robot_point_center, center_of_mass);*/ //TODO REMOVE COMMENT
    if(use_visualization)
    {
        // draw robot lines
        viewer->addLine(robot_point_0,robot_point_1,1.0,1.0,1.0,"f0");
        ...
        for (unsigned int i = 0; i < rating.size(); i++){
            if (check_each_axis || rating.at(i) < invalid_rating){ // instabil
                ...
                /*computeRobotPosition(support_point_1, support_point_2, support_point_3, p0, p1, p2, p3,
                                     ...
                                     normal_it,
                                     robot_point_0_it, robot_point_1_it, robot_point_2_it, robot_point_3_it,
                                     robot_point_mid_it, center_of_mass_it);*/ //TODO REMOVE COMMENT
