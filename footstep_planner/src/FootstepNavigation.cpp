// SVN $HeadURL: http://alufr-ros-pkg.googlecode.com/svn/trunk/humanoid_stacks/humanoid_navigation/footstep_planner/src/FootstepNavigation.cpp $
// SVN $Id: FootstepNavigation.cpp 4172 2013-05-21 11:24:18Z garimort@informatik.uni-freiburg.de $
...
FootstepNavigation::plan()
{
  ...
  if (ivPlanner.plan(status_flags))
  {
    startExecution();
    return true;
  }
...
FootstepNavigation::replan()
{
  ...
  uint32_t status_flags = 0;
  if (ivPlanner.replan(status_flags))
  {
    startExecution();
    return true;
  }
  ...
  {
    ROS_INFO("Replanning unsuccessful. Reseting previous planning information.");
	...
	const humanoid_nav_msgs::ExecFootstepsFeedbackConstPtr& fb)
{
  ...
  if (!getFootTransform(foot_id, ivIdMapFrame, ros::Time::now(), ros::Duration(0.5), &executed_tf))
  {
    /// @TODO: determine swing height and step duration
    State executed(executed_tf, 0.0, 0.0, 0.0, 0.0, planned.getLeg());
  ...
  if (executed_steps_idx >= ivControlStepIdx + 2)
    ...
    ROS_DEBUG("planned (%f, %f, %f, %i) vs. executed (%f, %f, %f, %i)",
              planned.getX(), planned.getY(), planned.getYaw(),
              planned.getLeg(),
              executed.getX(), executed.getY(), executed.getYaw(),
              executed.getLeg());
  ...
  const nav_msgs::OccupancyGridConstPtr& occupancy_map)
{
  ...
  if (ivPlanner.updateGroundLevelMap(map))
  {
    replan();
  }
}
...
bool
FootstepNavigation::setGoal(float x, float y, float z, float theta)
{
  return ivPlanner.setGoal(x, y, z, theta);
}
...
FootstepNavigation::updateStart()
{
  ...
  /// @TODO: determine swing height and step duration in case of backwards search
  State left(foot_left, 0.0, 0.0, 0.0, 0.0, LEFT);
  State right(foot_right, 0.0, 0.0, 0.0, 0.0, RIGHT);

  ROS_INFO("Robot standing at (%f, %f, %f, %f, %i) (%f, %f, %f, %f, %i).",
                       left.getX(), left.getY(), left.getZ(), left.getYaw(), left.getLeg(),
                       right.getX(), right.getY(), right.getZ(), right.getYaw(), right.getLeg());
				...
				humanoid_nav_msgs::StepTarget* footstep)
{
  ...
  tf::Transform step = from.inverse() *
                       tf::Pose(tf::createQuaternionFromYaw(to.getYaw()),
                                tf::Point(to.getX(), to.getY(), to.getZ()));
  ...
  else
  {
    ...
    float step_diff_theta = fabs(
        angles::shortest_angular_distance(
            tf::getYaw(from.getRotation()), from_planned.getYaw()));
    if (step_diff_x < ivAccuracyX && step_diff_y < ivAccuracyY &&
        step_diff_theta < ivAccuracyTheta)
    {
          step = tf::Pose(tf::createQuaternionFromYaw(from_planned.getYaw()),
                          tf::Point(from_planned.getX(), from_planned.getY(), from_planned.getZ())
                          ).inverse() *
                     tf::Pose(tf::createQuaternionFromYaw(to.getYaw()),
                                      tf::Point(to.getX(), to.getY(), to.getZ()));
  ...
  std::vector<humanoid_nav_msgs::StepTarget>& footsteps)
{
  ...
  tf::Pose last(tf::createQuaternionFromYaw(current_support_leg.getYaw()),
                tf::Point(current_support_leg.getX(), current_support_leg.getY(), current_support_leg.getZ()));
  const State* from_planned = to_planned.base();
  to_planned++;
  for (; to_planned != ivPlanner.getPathEnd(); to_planned++)
  {
    ...
    last = tf::Pose(tf::createQuaternionFromYaw(to_planned->getYaw()),
                    tf::Point(to_planned->getX(), to_planned->getY(), to_planned->getZ()));
    from_planned = to_planned.base();
  }
