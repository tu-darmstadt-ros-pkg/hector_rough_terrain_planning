// SVN $HeadURL: http://alufr-ros-pkg.googlecode.com/svn/trunk/humanoid_stacks/humanoid_navigation/footstep_planner/src/PathCostHeuristic.cpp $
// SVN $Id: PathCostHeuristic.cpp 3964 2013-02-19 12:17:18Z garimort@informatik.uni-freiburg.de $
...
: Heuristic(cell_size, num_angle_bins, PATH_STEP_COST_HEURISTIC),
  ivpGrid(NULL),
...
const
{
  ...
  if (ivDiffAngleCost > 0.0)
  {
    ...
    int diff_angle_disc = (
        ((to.getYaw() - current.getYaw()) % ivNumAngleBins) +
    ...
    diff_angle = std::abs(angles::normalize_angle(
        angle_cell_2_state(diff_angle_disc, ivAngleBinSize)));
  }
