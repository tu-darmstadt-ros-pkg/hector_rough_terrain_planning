...
TerrainPreprocessor::TerrainPreprocessor(const TerrainClassifierParams &params)
  : frame_id("/world")
  , lock_input_cloud(false)
{
  setParams(params);
...
void TerrainPreprocessor::setParams(const TerrainClassifierParams &params)
{
  this->params = params;
...
void TerrainPreprocessor::addCloud(const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &cloud, const std::string &frame_id)
{
  if (lock_input_cloud)
    return;
...
void TerrainPreprocessor::showHeight(pcl::visualization::PCLVisualizer &viewer, const std::string &name, int viewport) const
{
...
const pcl::PointCloud<pcl::PointXYZ>::ConstPtr &TerrainPreprocessor::getCloudInput() const
{
...
const pcl::PointCloud<pcl::PointXYZ>::Ptr &TerrainPreprocessor::getCloudProcessed() const
{
...
const pcl::PointCloud<pcl::PointXYZ>::Ptr &TerrainPreprocessor::getCloudOutfiltered() const
{
...
void TerrainPreprocessor::getCloudProcessedLowRes(pcl::PointCloud<pcl::PointXYZ>::Ptr &cloud) const
{
...
bool TerrainPreprocessor::computeNormals()
{
...
bool TerrainPreprocessor::computeHeight()
{
...
void TerrainPreprocessor::setDataOutdated()
{
  cloud_normals_outdated = true;
  cloud_gradients_outdated = true;
}

void TerrainPreprocessor::getGridMapIndex(const nav_msgs::OccupancyGrid::ConstPtr &map, float x, float y, int &idx) const
{
...
void TerrainPreprocessor::getGridMapCoords(const nav_msgs::OccupancyGrid::ConstPtr &map, unsigned int idx, float &x, float &y) const
{
...
float TerrainPreprocessor::planeDistance(const pcl::PointXYZ& testpoint, const pcl::PointXYZ& plane_n, const pcl::PointXYZ& plane_p)
{
...
pcl::PointXYZ TerrainPreprocessor::planeProjection(const pcl::PointXYZ& projection_p, const pcl::PointXYZ& plane_n, const pcl::PointXYZ& plane_p)
{
...
bool TerrainPreprocessor::atPlaneTest(const pcl::PointXYZ& testpoint, const pcl::PointXYZ& plane_n, const pcl::PointXYZ& plane_p, const float& delta)
{
...
pcl::PointXYZ TerrainPreprocessor::eval_point(const pcl::PointXYZ& tip_over_axis_point,
...
std::vector<pcl::PointXYZ> TerrainPreprocessor:: build_convex_hull(const pcl::PointCloud<pcl::PointXYZI>::Ptr cloud_positionRating,
...
void TerrainPreprocessor::compute_robot_positions(const pcl::PointXYZ support_point_1, const pcl::PointXYZ support_point_2, const pcl::PointXYZ support_point_3,
...
pcl::PointXYZ TerrainPreprocessor::compute_center_of_mass(const pcl::PointXYZ &p1_left,
...
float TerrainPreprocessor::computePositionRating(const pcl::PointXYZ& check_pos,
...
void TerrainPreprocessor::showPositionRating(pcl::visualization::PCLVisualizer &viewer, const std::string &name, int viewport) const{
    //draw
